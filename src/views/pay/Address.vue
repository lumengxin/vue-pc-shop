<template>
  <div class="address">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                ...
                >
                  <dl>
                    <dt>{{item.userName}} - {{item.addressId}}</dt>
                  ...
                  <!-- <div class="addr-opration" :class="{'addr-set-default': item.isDefault}"> -->
    ...
    <modal :mdShow="isMdShow" @close="closeModal">
      ...
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="delAddress">
          确认
        </a>
        <a class="btn btn--m btn--red" href="javascript:;" @click="isMdShow=false">
          取消
        </a>
      </div>
    </modal>
...
export default {
  ...
  methods: {
    async init() {
      let { data: res } = await Axios.get('/api/users/addressList')
      // let res = await Axios.get('/api/users/addressList')
      if (res.status === '0') {
    ...
    async setDefault(addressId) {
      console.log('setDefault -> addressId', addressId)
      let { data: res } = await Axios.post('/api/users/setDefault', { addressId: addressId })
      if (res.status === '0') {
    ...
    async delAddress() {
      let res = await Axios.post('/api/users/delAddress', {
        addressId: this.delAddress
      })
