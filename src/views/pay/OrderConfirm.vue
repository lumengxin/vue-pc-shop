              ...
              <li v-for="(item, index) in cartList" :key="index"
                ...
                <div class="cart-tab-1">
                  <div class="cart-item-pic">
                    <img v-lazy="require('../../../public/img/' + item.productImage)"
...
export default {
  ...
  methods: {
    async init() {
      const {data: res} = await Axios.get('/api/users/cartList')
    ...
    async payMent() {
      ...
      const {data: res} = await Axios.post('/api/users/payMent', {
        addressId: addressId,
